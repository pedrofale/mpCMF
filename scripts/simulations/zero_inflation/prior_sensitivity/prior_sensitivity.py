...
def main():	
	...
	silh_spca_scores = np.zeros((len(z_p_arr), len(eps_arr), n_runs))
	...
	for i in range(len(z_p_arr)):
		...
		for j in range(len(eps_arr)):
			...
			for k in range(n_runs):
				...
				# Run SparsePCA
				print('SPCA...')
				spca = SparsePCA(n_components=K).fit_transform(log_Y)
				...
				spca_silh = silhouette_score(spca, c_train)
				...
				silh_spca_scores[i, j, k] = spca_silh
	...
	np.save('{0}/silh_spca_scores.npy'.format(newpath), silh_spca_scores)
